...
def add_cumul_user_score(forecasts, scoring_rule, *args, **kwargs):
	forecasts=forecasts.groupby(['user_id']).apply(self.cumul_score, scoring_rule, *args, **kwargs)
...
def frontfill(forecasts):
	...
	forecasts=forecasts.groupby(['question_id', 'user_id', 'answer_option']).apply(frontfill_group)
...
def generic_aggregate(group, summ='arith', format='probs', decay='nodec', extremize='noextr', extrfactor=3, fill=False, expertise=False):
	n=len(group)
