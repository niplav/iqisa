...
def load_complete_markets(files=None, probmargin=0.005):
	...
	voidedquestions=questions.loc[questions['q_status']=='voided'][['q_type', 'question_id']]
	...
	for f in files:
		...
		if 'with_after_trade' in market_fixes[f]['fixes']:
			...
			market.loc[market['prob_after_trade']>=1, 'prob_after_trade']=1-probmargin
...
def load_complete_surveys(files=None, probmargin=0.005):
	...
	forecasts=pd.merge(forecasts, questions, on=['question_id', 'q_type'], suffixes=(None, '_x'))
